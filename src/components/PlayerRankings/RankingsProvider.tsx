...
function rankingsReducer(state: RankingsState, action: RankingsAction): RankingsState {
  switch (action.type) {
    ...
    case 'ADD_RANKED_PLAYER':
      ...
      return { ...state, rankedPlayers: newRanked.sort((a, b) => a.overall_rank - b.overall_rank) };
    case 'REMOVE_RANKED_PLAYER':
      const filtered = state.rankedPlayers.filter(p => p.player_id !== action.payload);
      // Update ranks
...
interface RankingsContextType {
  ...
  createSet: (name: string, format: 'dynasty' | 'redraft', copyFromSetId?: string) => Promise<void>;
...
export function RankingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchSets = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke('rankings-management');
  ...
  const createSet = useCallback(async (name: string, format: 'dynasty' | 'redraft', copyFromSetId?: string) => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      const { data, error } = await supabase.functions.invoke('rankings-management', {
  ...
  const selectSet = useCallback(async (setId: string) => {
    try {
      ...
      dispatch({ type: 'SET_RANKED_PLAYERS', payload: data.data.rankings || [] });
    } catch (error) {
  ...
  const saveRankings = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('rankings-reorder', {
  ...
  const createDefaultRankings = useCallback(async () => {
    ...
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      const { data, error } = await supabase.functions.invoke('rankings-default', {
  ...
  }, [state.availablePlayers, state.rankedPlayers, state.showOnlyUnranked, state.positionFilter, state.teamFilter, state.searchTerm]);
