...
export const CreateSetModal = forwardRef<CreateSetModalRef>((_, ref) => {
  ...
  const [rankingActiveTab, setRankingActiveTab] = React.useState<string>('overall');
  const [newRanking, setNewRanking] = React.useState<Player[]>([]);
  const [dragging, setDragging] = React.useState(false);
  const allPlayers = (state.availablePlayers && state.availablePlayers.length > 0)
    ? state.availablePlayers
    : MOCK_AVAILABLE_PLAYERS;

  // Helper to get a unique id for a player (use id if present, else name)
  const getPlayerId = (player: Player) => player.id ? String(player.id) : String(player.name);
  ...
  const filteredPlayers = allPlayers.filter((player: Player) => {
    ...
    const matchesPosition = activeTab === 'overall' || player.position === activeTab;
    const notInRanking = !newRanking.some(rp => getPlayerId(rp) === getPlayerId(player));
  ...
  const filteredRanking = newRanking
    .filter((player) => !filteredPlayers.some(ap => getPlayerId(ap) === getPlayerId(player)))
    .filter((player) => player.name.toLowerCase().includes(rankingSearchTerm.toLowerCase()))
    .filter((player) => rankingActiveTab === 'overall' || player.position === rankingActiveTab);
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === 'availablePlayers' && destination.droppableId === 'newRanking') {
    ...
    if (source.droppableId === 'newRanking' && destination.droppableId === 'newRanking') {
    ...
    if (source.droppableId === 'newRanking' && destination.droppableId === 'availablePlayers') {
      const updated = Array.from(newRanking);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={closeModal}>
      <DialogContent className={`sm:max-w-4xl${dragging ? ' no-transform' : ''}`}>
        <DialogHeader>
          <DialogTitle>Create New Ranking Set</DialogTitle>
          <DialogDescription>
            Build your custom ranking set by dragging players from the left to the right.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={e => { e.preventDefault(); /* Save logic here */ }} className="space-y-4">
          <div className="flex items-end gap-4">
            <div className="flex-1 flex flex-col">
              ...
              <Input
                id="name"
                value={form.name}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, name: e.currentTarget.value })}
            ...
            <div className="flex flex-row gap-8 mt-4">
              ...
              <div className="flex-1 flex flex-col gap-4">
                <div className="flex flex-wrap justify-center gap-2 mb-2">
                  {['overall', 'QB', 'RB', 'WR', 'TE', 'K', 'DEF'].map((position) => (
                    <button
                      key={position}
                      type="button"
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 flex items-center gap-2">
                          ...
                          <span className="ml-2 text-sm text-muted-foreground font-medium truncate">{player.team}</span>
                        </div>
                      </div>
                    );
                  }}
                >
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="flex flex-col gap-2 mt-4 max-h-[50vh] overflow-y-auto border rounded-lg p-2 bg-muted/30">
                      {filteredPlayers.map((player: Player, index: number) => (
                        <Draggable key={getPlayerId(player)} draggableId={getPlayerId(player)} index={index}>
                          {(provided) => (
                            <div
                              ref={provided.innerRef}
                              {...provided.draggableProps}
              ...
              <div className="flex-1 flex flex-col gap-4">
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRankingSearchTerm(e.currentTarget.value)}
                  placeholder="Search your ranking set by name..."
                  className="w-full h-12 text-base px-4 rounded-xl border border-border focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-300"
                  style={{ marginBottom: '0.5rem' }}
                />
                <Droppable droppableId="newRanking"
                  renderClone={(provided, _snapshot, rubric) => {
                    const idx = rubric.source.index;
                    const player = filteredRanking[idx] || filteredRanking[0] || { name: '', team: '', position: '' };
                    return (
                      <div
                        ref={provided.innerRef}
                        {...provided.draggableProps}
                ...
                >
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="flex flex-col gap-2 max-h-[50vh] overflow-y-auto border rounded-lg p-2 bg-muted/30 min-w-[300px]">
                      {filteredRanking.length === 0 && <div className="text-muted-foreground text-center py-8">Drag players here to build your set</div>}
