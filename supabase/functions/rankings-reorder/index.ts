...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Reordering rankings for set: ${reorderBody.setId}, user: ${user.id}`);

    if (!reorderBody.setId || !reorderBody.rankings || !Array.isArray(reorderBody.rankings)) {
      return new Response(
    ...
    const validRankings = reorderBody.rankings.filter(ranking => 
      ranking.playerId && 
    ...
    for (let i = 0; i < validRankings.length; i += batchSize) {
      ...
      try {
        ...
        if (upsertError) {
          ...
          console.log(`Successfully processed batch ${i / batchSize + 1} with ${batch.length} rankings`);
        }
      } catch (batchError) {
