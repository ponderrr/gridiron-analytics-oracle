...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Creating default rankings for set: ${requestBody.setId}, user: ${user.id}`
    );
    ...
    console.log(
      `Using trade values from Week ${currentWeek}, Season ${currentSeason}`
    );
    ...
    if (playersError) {
      console.error(
        "Error fetching players for consensus rankings:",
        playersError
      );

      // Fallback: get players without trade values
      const { data: fallbackPlayers, error: fallbackError } =
      ...
      return new Response(
        JSON.stringify({
          ...
          message:
            "Default rankings created using basic player order (no trade values available)",
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    for (let i = 0; i < rankings.length; i += batchSize) {
      ...
      try {
        ...
        if (insertError) {
          ...
          console.log(
            `Successfully inserted batch ${i / batchSize + 1} with ${
              batch.length
            } rankings`
          );
        }
