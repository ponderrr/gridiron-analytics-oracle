...
serve(async (req) => {
  ...
  try {
    const { week = getCurrentWeek(), season = 2024 } = await req.json().catch(() => ({}));
    
    console.log(`Starting weekly stats sync for Week ${week}, Season ${season}...`);
    ...
    if (playersError) {
      throw new Error(`Database error fetching players: ${playersError.message}`);
    }
    ...
    if (!statsResponse.ok) {
      throw new Error(`ESPN API error: ${statsResponse.status} ${statsResponse.statusText}`);
    }

    const statsData = await statsResponse.json();
    console.log('ESPN stats data structure preview:', JSON.stringify(statsData, null, 2).substring(0, 500));
    ...
    if (statsData.events && Array.isArray(statsData.events)) {
      for (const event of statsData.events) {
        try {
          ...
          errorDetails.push(`Error processing event ${event.id}: ${error.message}`);
    ...
    if (playerStats.size === 0) {
      ...
      await fetchPlayerStatsAlternative(season, week, playerStats, errorDetails);
    }
    ...
    for (const [espnPlayerId, stats] of playerStats) {
      try {
        ...
        if (!playerInfo) {
          errorDetails.push(`Player not found in database: ESPN ID ${espnPlayerId}`);
          continue;
        }

        // Calculate fantasy points using our existing function
        const fantasyPointsResponse = await supabase.functions.invoke('calculate-fantasy-points', {
          body: { 
            stats, 
            scoring_settings: {
              format: 'standard',
        ...
        errorDetails.push(`Error processing player ${espnPlayerId}: ${error.message}`);
    ...
    if (statsToInsert.length > 0) {
      ...
      for (let i = 0; i < statsToInsert.length; i += batchSize) {
        ...
        try {
          ...
          console.log(`Successfully upserted batch of ${batch.length} weekly stats`);
...
async function processTeamStats(competitor: any, playerStats: Map<string, WeeklyStatsInput>) {
    ...
    for (const player of competitor.roster) {
      if (player.athlete?.id && player.statistics) {
        ...
        const stats: WeeklyStatsInput = {
          ...
          passing_interceptions: extractStat(player.statistics, 'interceptions') || 0,
          rushing_yards: extractStat(player.statistics, 'rushingYards') || 0,
          rushing_tds: extractStat(player.statistics, 'rushingTouchdowns') || 0,
          receiving_yards: extractStat(player.statistics, 'receivingYards') || 0,
          receiving_tds: extractStat(player.statistics, 'receivingTouchdowns') || 0,
...
function extractStat(statistics: any[], statName: string): number {
  ...
  const stat = statistics.find(s => s.name === statName || s.abbreviation === statName);
  return stat ? parseFloat(stat.value) || 0 : 0;
}

// Alternative method to fetch player stats (fallback)
async function fetchPlayerStatsAlternative(season: number, week: number, playerStats: Map<string, WeeklyStatsInput>, errorDetails: string[]) {
  try {
